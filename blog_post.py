...
def create_wsse_header(username, api_key):
    ...
    return c.format(username,
                    b64encode(b_digest).decode(),
...
{2}
    ...
    """
    now = datetime.now()
    dtime = "{0}-{1}-{2}T{3}:{4}:{5}".format(now.year, now.month, now.day,
                                             now.hour, now.minute, now.second)
    data = template.format(title, username, body, dtime, category,
                           draft).encode()
    return data
...
def post_hatena(data, headers, blogname, username):
    url = 'http://blog.hatena.ne.jp/{0}/{1}/atom/entry'.format(
        username, blogname)
            ...
            for i in range(len(s)):
                ...
                if state == status.main_body:
                    ...
                    if s[i] == ')':
                        ...
                        print("""投稿に成功\n画像のidは[{0}:alt={1}]""".format(
                            imageid, alt_text))
                        blog_body += """[{0}:alt={1}]""".format(
                            imageid, alt_text)
                        photoname = ''
                        alt_text = ''
                    elif s[i] == '<':
...
def main():
    ...
    blog_title, blog_category, blog_body = translate_markdown_to_hatena_md(
        textname, headers)
    blog_data = translate_hatena_md_to_xml(blog_title, blog_category,
                                           blog_body, username, draft)
    print('markdown から xml への変換に成功')
    print('はてなブログへの投稿を開始')
    manage_url = 'https://blog.hatena.ne.jp/{0}/{1}/entries'.format(
        username, blogname)
